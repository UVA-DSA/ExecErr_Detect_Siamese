...
if type =="dual":
    ...
    for ith,net_type in enumerate(net_types):
        ...
        for G in all_g:
            data_dir=["",""] # Fill in the two data directories for suturing and needle passing
            ...
            for fold in range(len(unique_ids)):
                ...
                Task='all'
                cur_para = pkl.load(open('{}_para_{}/{}_para_{}_nested_all_{}.p'.format(G,net_type,G,net_type,fold),'rb'))
                find_para_dual(G,model,xtrain,ytrain,subject_train_ids,err_type_train,Task,fold,win_len,stride,net_type)
                config = {
            "lr":cur_para['lr'],
            ...
            "epoch":cur_para['epoch']}
                ...
                save_folder_model='{}_para_{}/'.format(G,net_type)
                if not os.path.exists(save_folder_model):
                    os.makedirs(save_folder_model,exist_ok=True)
                model_path = '{}_para_{}/{}_{}_model_all_{}.pth'.format(G, net_type, G,net_type,fold)
            ...
            save_folder='Siamese/GSTx/'
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)
                
            file_name='Siamese/GSTx/result_{}_{}.p'.format( net_type,G)
            pkl.dump(AllD, open(file_name,"wb"))
            print('finish_{}'.format(G))


        tb={'F1_mean': F1_mean, 'F1_std':F1_std, 'F1_v_mean':F1_v_mean,'F1_v_std':F1_v_std}
        df=pd.DataFrame(tb,index=all_g)

        df.to_csv('Siamese/GSTx/{}_input_F1_mean.csv'.format(net_type))
    ...
    for ith,net_type in enumerate(net_types):
        ...
        F_stds=[]
        for G in all_g:
            # if G!='G1' or net_type!='cnn':continue
