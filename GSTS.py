    ...
    for ith,net_type in enumerate(net_types):
        for Task in Tasks:
            ...
            if Task=="Suturing":
                ...
                data_dir=['',""] # todo fill in the data directoyr for suturing
            else: 
                all_g=["G1","G2","G3","G4","G6"]
                data_dir='' # todo fill in the data directoyr for needle passing
            ...
            for i, G in enumerate(all_g):
                ...
                for fold in range(len(unique_ids)):
                    ...
                    save_folder_model='Siamese/GSTS/{}_para_{}/'.format(G,net_type)
                    if not os.path.exists(save_folder_model):
                        os.makedirs(save_folder_model,exist_ok=True)
                    model_path = 'Siamese/GSTS/{}_para_{}/{}_{}_model_all_{}.pth'.format(G, net_type, G,net_type,fold)
                ...
                save_folder='Siamese/GSTS/{}'.format(net_type)
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                file_name='Siamese/GSTS/{}/{}_result_lstm_{}.p'.format(net_type,G, Task)
            ...
            df.to_csv('Siamese/GSTS/{}_input_F1_mean.csv'.format(net_type))
    ...
    for ith,net_type in enumerate(net_types):
        for Task in Tasks:
            ...
            for G in all_g:
                ...
                save_folder='Siamese/GSTS/'
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
            
                file_name='Siamese/GSTS/result_{}_{}.p'.format( net_type,G)
                pkl.dump(AllD, open(file_name,"wb"))
